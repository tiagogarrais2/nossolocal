...
} from "./AssemblableProductModal";

export default function ProductDetailClient({ product, store }) {
  ...
  const addToCart = async (customizationData = null) => {
    ...
    try {
      ...
      if (customizationData) {
        ...
        const customizations = { _observations: observations.trim() };
        body = {
          productId: product.id,
          quantity: 1,
          customizations,
          customizationHash: computeCustomizationHash(customizations),
        };
      } else {
